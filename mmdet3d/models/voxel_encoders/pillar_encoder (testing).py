...
class DynamicPillarFeatureNet(PillarFeatureNet):
    ...
    def __init__(self,
        ...
        # NOTE: Test for the dynamicization, do not know if it is needed or not
    ...
    def map_voxel_center_to_point(self, pts_coors: Tensor, voxel_mean: Tensor,
        ...
        # Needed to change parameter here: the batch size is obtained correctly only in this way!!
        ...
        # TODO: Reference prints
        print("\n\nCanvas channel")
        print(canvas_channel)
        print(type(canvas_channel))
        print("\nBatch size")
        print(batch_size)
        print(type(batch_size))
        print("\nCanvas len")
        print(canvas_len)
        print(type(canvas_len))
        ...
        print(f"The dimension of indices is: {indices.shape}")
        ...
        voxel_index = (
            voxel_mean[:, 0] * canvas_y * canvas_x +
            voxel_mean[:, 2] * canvas_x + voxel_mean[:, 3])




        print(f"The dimension of voxel_index is: {voxel_index.shape}")
    ...
    def forward(self, features: Tensor, coors: Tensor) -> Tensor:
        ...
        #   - Det3DDataPreprocessor is called from, and gives the data to, the VoxelNet class
        #   - the class VoxelNet calls this encoder, and passes the right arguments
        # 
        # 
        # The output of the Det3DDataPreprocessor in dynamic mode is a dict with the following keys
        #   - "points" with a list of length=batch_size and each element is a tensor of shape (N_points x 4)
        #   - "voxels"
        #       - "voxels" which is a tensor of shape (N_points_total x 4) (it's all the points of the batch, the concatenation of 
        #         the two tensors in the "points" key)
        #       - "coors" which is a tensor of shape (N_voxels_total x 4) where the first number of the four indicates the image
        #         source inside the batch
        # 
        # 
        # The arguments passed to this encoder are then:
        #   - a tensor containing all the voxels of the batch (even if batch_size != 1)
        #   - a tensor containing all the coords of the batch
        #       - even if batch_size != 1
        #       - the first number of the coords, correspond to the position in the batch
        ...
        if self._with_cluster_center:
            ...
            # TODO: Reference prints
            print(f"\n\nVoxel_mean from the DynamicScatter3D:\n{voxel_mean}\nof shape {voxel_mean.shape}")
            print(f"\n\nMean_coors from the DynamicScatter3D:\n{mean_coors}\nof shape {mean_coors.shape}\n\n")
            ...
            points_mean = self.map_voxel_center_to_point(
                features[:, :3], voxel_mean, mean_coors)
            ...
            #     print(element.shape)
            # print("\n\n\n")
            # print(type(points_mean))
            # print(points_mean)
            # print(points_mean.shape)
            # print("\n\n\n")


            # features = torch.cat((features[0], features[1]), dim=0)           # NOTE: Actually should be done based on the batch size


            # TODO: maybe also do cluster for reflectivity -----> NOTE: What do we do with this?? Also check what they do in 
            #                                                           PillarFeatureNet (the non-dynamic one)
