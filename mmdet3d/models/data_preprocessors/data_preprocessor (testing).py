    ...
    def voxelize(self, points: List[Tensor],
        ...
        if self.voxel_type == 'hard':
            ...
            for i, res in enumerate(points):
                ...
                # Needed change: before here it was "res_coors = self.voxel_layer(res)"
                # res_voxels, res_coors, res_num_points = self.voxel_layer(res)
                res_coors = self.voxel_layer(res)

                # More information about the output of the preprocessor is given in the next step, the "DynamicPillarFeatureNet" of
                # the file "pillar_encoder.py" in the function "forward"
                # 
                # NOTE:
                #   - The iteration happens at batch level, NOT at point level
                #   - The "res_voxels" and "res_num_points" are not used
                #   - The voxel_layer is a "VoxelizationByGridShape" and is exactly the same as
                #     the "hard" voxelization type. Maybe all the changes are made later by the encoder?
