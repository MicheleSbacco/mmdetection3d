...
class VoxelNet(SingleStage3DDetector):
    ...
    def extract_feat(self, batch_inputs_dict: dict) -> Tuple[Tensor]:
        ...
        for key in batch_inputs_dict:
            print(f"{key}:\ttype is {type(batch_inputs_dict[key])}")
            if isinstance(batch_inputs_dict[key], dict):
                for subkey in batch_inputs_dict[key]:
                    print(f"\tsubkey {subkey} of type {type(batch_inputs_dict[key][subkey])} with shape {batch_inputs_dict[key][subkey].shape}")
            else:
                for element in batch_inputs_dict[key]:
        ...
        if not is_dynamic:
            # This is the version for "hard" voxelization
            voxel_dict = batch_inputs_dict['voxels']
            voxel_features = self.voxel_encoder(voxel_dict['voxels'],
                                                voxel_dict['num_points'],
                                                voxel_dict['coors'])
        else:
            # This is the version for "dynamic" voxelization. Similar to what is done for "hard" voxelization but
            #   - (SILLY) does not pass through the intermediate dictionary voxel_dict
            #   - (RELEVANT) passes one less parameter
            # 
            #   - NOTE: more details are explained in the encoder file (DynamicPillarFeatureNet in file pillar_encoder.py)
            voxel_dict = batch_inputs_dict['voxels']
            voxel_features, feature_coors = self.voxel_encoder(voxel_dict['voxels'],
                                                voxel_dict['coors'])
        

        print(voxel_features.shape)
        print(feature_coors.shape)
